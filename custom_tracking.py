...
chosen_tracker = widgets.Select(
    options=["CentroidTracker", "IOUTracker"],
    value='CentroidTracker',
    rows=5,
    description='MOTracking:',
    disabled=False
)

if chosen_tracker.value == 'CentroidTracker':
    tracker = CentroidTracker(max_lost=100, tracker_output_format='mot_challenge')
elif chosen_tracker.value == 'IOUTracker':
    tracker = IOUTracker(max_lost=100, iou_threshold=0.5, min_detection_confidence=0.8, max_detection_confidence=0.9,
...
for i in range(0,numFrames,frameSkip):
    ...
    # tracking
    tracks = tracker.update(filtered_bboxs,filtered_scores,filtered_class_ids)
    for t in tracks:
        print(t)
    print("--------------------------------------")

    # Draw tracks
    result_image = draw_tracks(result, tracks)
    replicated_mask_image = draw_tracks(replicated_mask_, tracks)


    display_image = np.hstack((result_image, replicated_mask_image))
    final_image = cv2.resize(display_image, (desired_width, desired_height))


    # Add a final label to the image
    title_label = "People Tracking in Dancing Occlusion"
    count_label = f"Detection Count : [ {len(filtered_bboxs)} ] "

    cv2.putText(final_image, title_label, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
    cv2.putText(final_image, count_label, (10, 80), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
